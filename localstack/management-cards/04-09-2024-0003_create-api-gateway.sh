...
CARDS_RESOURCE_ID=$(awslocal apigateway get-resources \
                  --rest-api-id $API_GATEWAY_ID \
                  --query "items[?path=='/api/v1/cards'].[id]" \
                  --output text)
...
awslocal lambda add-permission \
    --function-name "${MANAGEMENT_CARDS_LAMBDA_NAME}" \
    --source-arn "arn:aws:execute-api:us-east-1:000000000000:${API_GATEWAY_ID}/*/POST/api/v1/cards" \
